...
def get_work_orders(db: Session = Depends(get_db)):
    ...
    for order in orders:
        ...
        result.append(WorkOrderResponse(
            ...
            service={"id": service.id, "name": service.name, "price": float(service.price) if service.price else 0} if service else {},
            status=order.status,
...
def create_work_order(data: WorkOrderCreate, db: Session = Depends(get_db)):
    ...
    return WorkOrderResponse(
        ...
        service={"id": order.service_id, "name": service.name if service else "", "price": float(service.price) if service and service.price else 0},
        status=order.status,
        total_cost=float(order.total_cost or 0),
        description=order.description,
        scheduled_date=order.scheduled_date.isoformat() if order.scheduled_date else None,
